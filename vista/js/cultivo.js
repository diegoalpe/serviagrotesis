...
function listar(){
    ...
    $.post
    (
        "../controlador/cultivo.listar.controlador.php",
        {
...
function eliminar(codigoCultivo){
   swal({
            title: "Confirme",
	...
	function(isConfirm){
            if (isConfirm){
                $.post(
                    "../controlador/cultivo.eliminar.controlador.php",
                    {
                        p_codigo_cultivo: codigoCultivo
                    }
                    ).done(function(resultado){
                        var datosJSON = resultado;   
...
$("#frmgrabar").submit(function(evento){
	...
	function(isConfirm){ 
            ...
            if (isConfirm){ //el usuario hizo clic en el boton SI     
                ...
                $.post(
                    "../controlador/cultivo.agregar.editar.controlador.php",
                    {
...
$("#btnagregar").click(function(){
    ...
    $("#titulomodal").text("Agregar nuevo Cultivo");
...
function leerDatos( codigoCultivo ){
    
    $.post
        (
            "../controlador/cultivo.leer.datos.controlador.php",
            {
                p_codigo_cultivo: codigoCultivo
            }
        ).done(function(resultado){
            ...
            if (datosJSON.estado === 200){
                ...
                $.each(datosJSON.datos, function(i,item) {
                    $("#txtcodigo").val( item.codigo_cultivo );
                    $("#txtnombre").val( item.nombre );
                    $("#txtsemilla").val( item.semilla );
                    $("#cboagricultormodal").val( item.codigo_agricultor );
                    
                    //Ejecuta el evento change para llenar las categorías que pertenecen a la linea seleccionada
                    $("#cboagricultormodal").change();
